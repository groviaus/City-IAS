...
export function validateEmail(email) {
  ...
  const commonDomains = ['gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com', 'rediffmail.com', 'live.com'];
  
  if (!commonDomains.includes(domain) && !domain.includes('.co.in') && !domain.includes('.in')) {
    return { isValid: false, error: 'Please enter a valid email address' };
  }
...
/**
 * Validates course field - only text allowed (no numbers)
...
export function validateCourse(course) {
  ...
  // Check if course contains only letters, spaces, and common punctuation
  const courseRegex = /^[A-Za-z\s\-&()]+$/;
  if (!courseRegex.test(cleanCourse)) {
    return { isValid: false, error: 'Course can only contain letters, spaces, and basic punctuation' };
  }
  
  // Check minimum length
  if (cleanCourse.length < 3) {
    return { isValid: false, error: 'Course must be at least 3 characters long' };
  }
...
export function validateCityState(cityState) {
  ...
  if (!cityStateRegex.test(cleanCityState)) {
    return { isValid: false, error: 'City and State can only contain letters, spaces, and basic punctuation' };
  }
  
  // Check minimum length
  if (cleanCityState.length < 3) {
    return { isValid: false, error: 'City and State must be at least 3 characters long' };
  }
  
  // Check maximum length
  if (cleanCityState.length > 100) {
    return { isValid: false, error: 'City and State cannot exceed 100 characters' };
  }
